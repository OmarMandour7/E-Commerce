...
const AdminEditProductsHook = (id) => {
    ...
    const [priceBefore, setPriceBefore] = useState('PriceBefore');
    const [priceAftr, setPriceAftr] = useState('priceAfter');
    ...
    useEffect(() => {
        try{ if (item.data) {
            ...
            setSubCatID(item.data.subcategory[0])
        ...
        if (loading === false) {
            ...
            if (product) {
                if (product.status === 200) {
                    notify("Product Edited Succesfully","success")
                } else {
